...
const BookAppointment1 = ({ history }) => {
    /**
     * Set State
     */
    ...
    // console.log("....treate", treatmentarea);
    const newappointments = useSelector((state) => state.appointmentReducer);
    const { loading } = newappointments;
    useEffect(() => {
    ...
    useEffect(async () => {
        ...
        if (!userInfo) {
            ...
            // UserInfo
            // if (!userInfo || !userInfo.firstName || userInfo.id !== userInfo.id) {
            //   dispatch(getUserDetails(userInfo.id));
            let data = await dispatch(getIncompleteAppointments());
            if (!data) {
                data = userInfo;
            }
    ...
    const submitHandler = async () => {
        const utcPreferredTime = formatInTimeZone(
            zonedTimeToUtc(appointmentDateTime, userInfo?.timezoneName?.abbrevation),
            "UTC",
            "yyyy-MM-dd HH:mm"
        );
        ...
        const status = await dispatch(
            createAppointment({
                ...
                treatmentArea: treatmentArea?.map((data) => data),
                // treatmentArea: ["6243f4fdf7d9bc608c7cb776", "6243f4fdf7d9bc608c7cb776"]
            })
        );
        history.push("/add-pharmacy");
        if (status) {
